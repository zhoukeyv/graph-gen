...
function initNetwork() {
    ...
    network.on("oncontext", function (params) {
        ...
        if (nodeId !== undefined) { network.selectNodes([nodeId]); showNodeContextMenu(nodeId, params.event.clientX, params.event.clientY); } 
...
window.formatAsTree = function() {
    ...
    setTimeout(() => { 
        let positions = network.getPositions(); network.setOptions({ layout: { hierarchical: { enabled: false } } }); 
        nodesDataset.update(nodesDataset.get().map(node => { let style = getStyleObject(node, { isPinned: true }); if (positions[node.id]) { style.x = positions[node.id].x; style.y = positions[node.id].y; } return style; })); 
        window.forcePhysicsUpdate(); network.fit({ animation: { duration: 600 } }); 
    }, 50);
}
...
window.renderGraphFromText = function() {
    ...
    edgesDataset.clear(); edgesDataset.add(parsedEdges); 
...
function getEdges_Tree(n) { let edges = []; for (let i = 2; i <= n; i++) edges.push([i, rand(1, i - 1)]); return edges; }
...
function getEdges_Binary(n) { let edges = []; for (let i = 2; i <= n; i++) edges.push([Math.floor(i / 2), i]); return edges; }
function getEdges_Graph(n, m, isDirected) { let edges = [], existing = new Set(); for (let i = 2; i <= n; i++) { let u = i, v = rand(1, i - 1); if(!isDirected && u > v) { let t = u; u = v; v = t; } edges.push([u, v]); existing.add(`${u}-${v}`); } let attempts = 0; while (edges.length < m && attempts < m * 5) { attempts++; let u = rand(1, n), v = rand(1, n); if (u === v) continue; if (!isDirected && u > v) { let t = u; u = v; v = t; } let key = `${u}-${v}`; if (!existing.has(key)) { edges.push([u, v]); existing.add(key); } } return edges; }
...
window.copyOutput = function() { navigator.clipboard.writeText(document.getElementById('output').value).then(() => { const fb = document.getElementById('copyFeedback'); fb.style.opacity = '1'; setTimeout(() => fb.style.opacity = '0', 2000); }); }

...
window.renderGraphFromText = function() {
    ...
    let isDirected = document.getElementById('isDirected').value === 'true'; network.setOptions({ edges: { arrows: { to: { enabled: isDirected } } } });
    const text = document.getElementById('output').value.trim(); if (!text) { nodesDataset.clear(); edgesDataset.clear(); return; }
    const lines = text.split('\n').map(l => l.trim()).filter(l => l !== ''); if (lines.length === 0) return;
    const n = lines[0].split(/\s+/).map(Number)[0]; if (isNaN(n) || n > 2000) return; 
    let newNodes = []; for (let i = 1; i <= n; i++) { let existingNode = nodesDataset.get(i); newNodes.push(existingNode ? existingNode : getStyleObject({ id: i }, { label: String(i) })); }
    nodesDataset.remove(nodesDataset.getIds().filter(id => id > n)); nodesDataset.update(newNodes);
    let newEdges = []; for (let i = 1; i < lines.length; i++) { const parts = lines[i].split(/\s+/); if (parts.length >= 2 && !isNaN(parts[0]) && !isNaN(parts[1])) { let edge = { from: parseInt(parts[0]), to: parseInt(parts[1]) }; if (parts.length >= 3) { edge.label = parts.slice(2).join(' '); } newEdges.push(edge); } }
    edgesDataset.clear(); edgesDataset.add(newEdges); setTimeout(() => { window.forcePhysicsUpdate(); }, 10);
}
...
window.formatAsTree = function() {
    ...
    let rootId = parseInt(document.getElementById('treeRootInput').value);
    if (isNaN(rootId) || !nodesDataset.get(rootId)) { alert(`未找到节点 [${rootId}]！`); return; }
    ...
    if (contextEdgeId !== null) {
        ...
        if (edge) {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (parts.length >= 2) { let u = parseInt(parts[0]), v = parseInt(parts[1]); let match = isDirected ? (u == edge.from && v == edge.to) : ((u == edge.from && v == edge.to) || (u == edge.to && v == edge.from)); if (match) { lines[i] = newWeight ? `${u} ${v} ${newWeight}` : `${u} ${v}`; break; } }
            }
